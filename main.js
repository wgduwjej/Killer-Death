...
const starts = async (pebz = new WAConnection()) => {
  ...
  pebz.version = [2, 2123, 8];
  pebz.browserDescription = ["hehe boy", "Chrome", "3.0"];
  ...
  pebz.on("qr", () => {
    console.log(
      ...
      color("]", "white"),
      color(" Scan bang"),
      pebz.sendMessage(`6285774523785@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Berhasil Terhubung!_`, MessageType.extendedText)
  ...
  pebz.on("open", () => {
    success("2", "Connect Sukses > Follow @IMpoSIbbLE990");
...
console.log(color(`${spc3}[•]`, 'aqua'), color(`Owner       : I-AM BOOST}`, 'white'))
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('[SELF-BOT]'), color('𝗦𝗲𝗹𝗳𝗯𝗼𝘁 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await pebz.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/Gejrkejekdbskn`
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'ɪ-ᴀᴍ ʙᴏᴏsᴛ シ︎⁩',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Rival;;;\nFN:resku\nitem1.TEL;waid=6285774523785:+62 857-7452-3785\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
metdata = await pebz.groupMetadata(anu.jid)
...
console.log(`- [ Group Setting Change ] - In ${metdata.subject}`)
  ...
  pebz.on("group-participants-update", async (anu) => {
    try {
      ...
      console.log(anu);
      try {
        pp_user = await pebz.getProfilePicture(mem);
      } catch (e) {
        pp_user =
      ...
      if (anu.action == "add" && mek.includes(pebz.user.jid)) {
        pebz.sendMessage(anu.jid, "HALO BEB 👋", "conversation");
      }
...
if (anu.action == 'add' && !mek.includes(pebz.user.jid)) {
             ...
             const thu = await pebz.getStatus(anu.participants[0], MessageType.text)
             const num = anu.participants[0]
             const bosco1 = await pebz.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                welcomeBut = [{buttonId:`${prefix}getdeskgc`,buttonText:{displayText:'DESK GC'},type:1}, {buttonId:`${prefix}owner1`,buttonText:{displayText:'OWNER'},type:1}]
                welcomeButt = { contentText: ` `, footerText: `${teks}`, buttons: welcomeBut, headerType: 6, locationMessage: bosco2.message.locationMessage}
                pebz.sendMessage(mdata.id, welcomeButt, MessageType.buttonsMessage, { caption: 'hehe', "contextInfo": { "mentionedJid" : [num], },})
                 }
            if (anu.action == 'remove' && !mem.includes(pebz.user.jid)) {
                const mdata = await pebz.groupMetadata(anu.jid)
                const num = anu.participants[0]
                const bosco3 = await pebz.prepareMessage("0@s.whatsapp.net", hehe, MessageType.location,{ thumbnail: hehe})
                ...
                out = `*Dadah Beb* 👋\n@${num.split('@')[0]}\n *Semoga Tenang* 🌹\n${time_wel} -  ${time_welc}`
                goodbyeBut = [{buttonId:`${prefix}h`,buttonText:{displayText:'PERGI KELUAR'},type:1}, {buttonId:`${prefix}owner1`,buttonText:{displayText:'OWNER'}, type:1}]
                goodbyeButt = { contentText: ` `, footerText: `${out}`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage}
                pebz.sendMessage(mdata.id, goodbyeButt, MessageType.buttonsMessage, { caption: 'hehe', "contextInfo": { "mentionedJid" : [num], },})
        }
      /*welcom = true
      if (anu.action == "add" && !mem.includes(pebz.user.jid)) {
        ...
        imageMsg = (
          await pebz.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
          })
        ).imageMessage;
        buttonsMessage = {
        ...
        prep = await pebz.prepareMessageFromContent(
          mdata.id,
          { buttonsMessage },
          {}
        );
      ...
      if (anu.action == "promote") {
        ...
        anu_user = w.vname || w.notify || num.split("@")[0];
        try {
          ppimg = await pebz.getProfilePicture(
        ...
        teks = `${anu_user} SELAMAT YA BEBAN︎`;
        pebz.sendMessage(mdata.id, buffer, MessageType.image, {
          caption: teks,
        });
      }
      ...
      if (anu.action == "demote") {
        ...
        teks = `${anu_user} SURAM DI DEMOTE︎`;
        pebz.sendMessage(mdata.id, buffer, MessageType.image, {
          caption: teks,
        });
      }
  ...
  //
 antidel = true
  pebz.on("message-delete", async (m) => {
  ...
  •> Type : ${type}`,
    ...
    pebz.copyNForward(m.key.remoteJid, m.message);
  });
  ...
  isBattre = "Not Detect"; //
  isCharge = "Not Detect"; //
  pebz.on(`CB:action,,battery`, (json) => {
